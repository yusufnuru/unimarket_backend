...
import { Profiles, Users, Sessions, VerificationCodes } from '../schema';
import { compareValue, hashValue } from '../utils/argon2';
import { APP_ORIGIN, CONFLICT, INTERNAL_SERVER_ERROR, NOT_FOUND, TOO_MANY_REQUESTS, UNAUTHORIZED, VerificationCodeType } from '../constants';
import {fiveMinutesAgo, ONE_DAY_MS, oneHourFromNow, oneYearFromNow, thirtyDaysFromNow} from '../utils/date';
import appAssert from '../utils/appAssert';
import { RefreshTokenPayload, refreshTokenSignOptions, signToken, verifyToken } from '../utils/jwt';
import { Email, LoginUserDto, RegisterUserDto, VerificationCode } from './authSchema';
...
export const refreshUserAccessToken = async (refreshToken: string) => {
  ...
  const now = Date.now();
  appAssert(session
    && new Date(session.expiresAt).getTime() > now, UNAUTHORIZED, 'Session expired');
  ...
  const newRefreshToken = sessionNeedsRefresh ? signToken({sessionId, role}, refreshTokenSignOptions) : undefined;
...
export const verifyEmail = async (code: VerificationCode) => {
  ...
  const validVerificationCode = await db.query.VerificationCodes
    .findFirst({
  ...
  const updatedUser = await db.update(Users).set({verified: true})
...
export const resetPassword = async (request: ResetPasswordParams) => {
  // get the verification code
  const vaildCode = await db.query.VerificationCodes.findFirst({
    where: (VerificationCodes, { eq, and, gt}) =>
      and(
  ...
  appAssert(vaildCode, NOT_FOUND, 'Invalid OR expired verification code');
  ...
  const [updatedUser] = await db.update(Users)
    ...
    .where(eq(Users.id, vaildCode.userId))
    .returning({
      id: Users.id,
      email: Users.email,
    });
